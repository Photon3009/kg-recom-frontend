...
import { uploadResume } from '@/lib/api/services/candidateService';
import type { CandidateUploadResponse } from '@/lib/types/candidate';
import { SkillBadge } from '@/components/ui/SkillBadge';

export default function ResumeUploadPage() {
  const router = useRouter();
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [uploadResult, setUploadResult] = useState<CandidateUploadResponse | null>(null);
  ...
  const handleUpload = async (file: File) => {
    try {
      ...
      setTimeout(() => {
        router.push(`/recommendations/${result.data.candidate_id}`);
      }, 2000);
    } catch (err) {
      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Upload your resume in PDF, TXT, or DOCX format. Our AI will extract your skills,
            experience, and education to find matching jobs.
          </p>
        </div>
                ...
                <div className="ml-3 flex-1">
                  ...
                  <div className="mt-4 space-y-3">
                    <div>
                      ...
                      <p className="text-green-900">{uploadResult.data.name}</p>
                    </div>
                    <div>
                    ...
                    <div className="mt-4 text-sm text-green-800">
                      ...
                      <p className="mt-1">Finding matching jobs...</p>
                    </div>
                  </div>
                </div>
              </div>
